...
export function NotionCommentImportForm({
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Notion에서 가져오기</CardTitle>
        <CardDescription>
          Notion 데이터베이스의 각 행을 학생 댓글 하나로 가져옵니다. 이미 가져온 행은 자동으로 건너뜁니다.
        </CardDescription>
      </CardHeader>
      <CardContent>
        {configured ? null : (
        ...
        <form action={importCommentsFromNotion} className="space-y-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="content_property">댓글 내용 속성</Label>
            <Input
              id="content_property"
          ...
          <Button type="submit">Notion에서 가져오기</Button>
        </form>
      </CardContent>
    </Card>
  );
