...
function getModel(modelOverride?: string) {
  return modelOverride?.trim() || process.env.OPENAI_QUESTIONING_MODEL || process.env.OPENAI_EVALUATION_MODEL || "gpt-5.6";
}

function getApiKey(apiKeyOverride?: string) {
  const key = apiKeyOverride?.trim() || process.env.OPENAI_API_KEY;
  if (!key) {
