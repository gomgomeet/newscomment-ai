...
import { ArrowLeft, CheckCircle2, CircleDashed, FileDown, Send, Sparkles } from "lucide-react";
import { exportStudentSummaryToNotion, openStudentSummary, saveStudentSummary } from "@/app/dashboard/growth/actions";
...
import { parseStudentRecordEvidence } from "@/lib/growth/student-records";
...
export default async function StudentSummaryPage({
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6">
      ...
      <section className="rounded-md border border-teal-200 bg-teal-50/60 p-5"><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-start"><div><p className="text-sm font-semibold text-teal-700">과목별 세특 작성</p><div className="mt-1 flex flex-wrap items-center gap-2"><h2 className="text-2xl font-semibold">{summary.student_key}</h2>{recordEvidence?.subject ? <span className="rounded-full bg-white px-3 py-1 text-sm font-semibold text-teal-800">{recordEvidence.subject}</span> : null}</div><p className="mt-2 text-sm text-muted-foreground">교사 확정 평가 {summary.included_evaluation_ids.length}건 · {recordEvidence?.activityRecords.length ?? 0}개 활동 근거</p></div><div className="flex flex-wrap items-center gap-2"><form action={openStudentSummary}><input type="hidden" name="student_key" value={summary.student_key} /><input type="hidden" name="subject" value={recordEvidence?.subject ?? ""} /><input type="hidden" name="period_label" value={summary.period_label} /><input type="hidden" name="regenerate" value="true" /><Button type="submit" size="sm" variant="outline"><Sparkles className="h-4 w-4" /> AI 초안 다시 만들기</Button></form><span className="flex h-fit items-center gap-1 rounded-full bg-white px-3 py-1.5 text-sm font-semibold">{summary.status === "confirmed" ? <CheckCircle2 className="h-4 w-4 text-teal-600" /> : <CircleDashed className="h-4 w-4 text-amber-600" />}{summary.status === "confirmed" ? "교사 확정" : "초안"}</span></div></div></section>
      {recordEvidence ? <section className="space-y-3"><div><h3 className="text-lg font-semibold">활동별 근거</h3><p className="mt-1 text-sm text-muted-foreground">{recordEvidence.subject} 교과의 통합 세특을 작성할 때 참고하는 교사 확정 근거입니다.</p></div><div className="overflow-hidden rounded-md border bg-card"><div className="hidden grid-cols-[220px_minmax(360px,1fr)_120px] gap-4 border-b bg-muted/50 px-4 py-3 text-xs font-semibold text-muted-foreground md:grid"><span>활동명</span><span>관찰·평가 근거</span><span>상태</span></div>{recordEvidence.activityRecords.map((record) => <div key={record.projectId} className="grid gap-3 border-b px-4 py-4 last:border-b-0 md:grid-cols-[220px_minmax(360px,1fr)_120px]"><div><p className="font-semibold">{record.activityTitle}</p><p className="mt-1 text-xs text-muted-foreground">{record.evidenceSummary}</p></div><p className="text-sm leading-6">{record.recordText}</p><div><span className="text-xs text-muted-foreground">활동 근거</span></div></div>)}</div></section> : null}
      <details className="rounded-md border bg-card"><summary className="cursor-pointer px-4 py-3 font-semibold">교사 확정 평가 근거 보기</summary><div className="space-y-3 border-t p-4">{(evaluationsResult.data ?? []).map((evaluation, index) => { const comment = commentById.get(evaluation.comment_id); return <div key={evaluation.id} className="border-b pb-4 last:border-b-0 last:pb-0"><div className="flex justify-between gap-3"><div><p className="font-semibold">{index + 1}. {projectById.get(evaluation.project_id)?.title || "평가활동"}</p><p className="mt-1 text-xs text-muted-foreground">{evaluation.confirmed_at ? new Date(evaluation.confirmed_at).toLocaleString("ko-KR") : "확정 시각 없음"}</p></div><span className="text-xs font-semibold text-teal-700">교사 확정</span></div><p className="mt-3 rounded-md bg-muted/50 p-3 text-sm leading-6">{comment?.content || "원결과물 없음"}</p><p className="mt-2 text-sm"><span className="font-medium">교사 피드백 · </span>{evaluation.feedback || "없음"}</p></div>; })}</div></details>
      ...
      <Card><CardHeader><CardTitle>{recordEvidence?.subject || "과목"} 통합 세특</CardTitle><CardDescription>같은 과목의 활동 근거를 종합한 초안입니다. 교사가 검토·수정하고 확정해야 내보낼 수 있습니다.</CardDescription></CardHeader><CardContent className="space-y-5"><div className="rounded-xl bg-muted/50 p-4"><p className="text-xs font-semibold text-muted-foreground">과목별 통합 초안</p><p className="mt-2 whitespace-pre-wrap text-sm leading-7">{summary.draft_text}</p></div><form action={saveStudentSummary} className="space-y-4"><input type="hidden" name="summary_id" value={summary.id} /><div className="space-y-2"><Label htmlFor="teacher_final_text">교사 최종 문장</Label><Textarea id="teacher_final_text" name="teacher_final_text" rows={9} defaultValue={summary.teacher_final_text} /></div><div className="flex flex-wrap justify-end gap-2"><Button type="submit" name="confirm" value="false" variant="outline">초안 저장</Button><Button type="submit" name="confirm" value="true">교사 최종 확정</Button></div></form></CardContent></Card>
