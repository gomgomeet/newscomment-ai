...
## 2026-08-26 A·B·C·D 완료

- **A** Notion 루브릭 동적 연결, 세특 작성 프롬프트, 더 알아볼 질문 기록을 연결했다.
- **B** Gemini·로컬·단일 HTML에 사실 보존형 어려운 문장 풀이를 적용했다.
- **C** B1/B2와 이해→성취기준→의견 순서의 2국면 대화를 대화 이력으로 구현했다.
- **D** 질문하기·지문 이해·성취기준·성찰질문과 의견 표현을 0~5점으로 판정한다.
- 교사용 `평가 불러오기`는 Notion 실제 판정값을 우선해 표와 엑셀을 채우고, 저장값이 없는 학생에게만 Gemini 추천값을 사용한다.
- 최신 회귀평가는 **49세션, 207턴, 실패 0건, 검토 플래그 0건**이다.
- 운영 배포 전 Notion 결과 DB에 `질문하기`, `지문 이해`, `성취기준 점수`, `성찰질문과 의견 표현`, `도달 난이도`, `더 알아볼 질문` 열이 있어야 한다.
- 2026-08-26 읽기 전용 점검에서 현재 운영 결과 DB에 위 6개 열이 없음을 확인했으므로, 열 추가와 Preview 실저장 검증 전까지 Production 승격을 보류한다.

이번 변경의 GitHub PR 본문과 배포 게이트는 `docs/QUESTIONING_CHATBOT_ABCD_PR.md`에 정리했다.
...
- `evals/questioning-chatbot/fixtures/sessions.json`: 개선에 사용하는 개발 세트 39회기, 167턴
...
최초 확장 실행은 개발 세트에서 실패 19건과 검토 플래그 95건을 발견했다. 오류 유형을 프롬프트와 일반 대화정책에 반영하고 두 국면 평가 세션을 추가했다. 최종 자동 평가 결과는 **전체 49회기, 207턴, 실패 0건, 검토 플래그 0건**이다. 한 응답 최대 한 질문, 내부 정보 비노출, 종료 일관성, 관계 회복, 복사 요구, 인과 과장, 반복 응답, 기계적 문구와 어휘 답변의 사전적·문맥적 근거를 검사했다.

평가 요청에서 별도 축약 동작 설정을 제거해, 교사용 제작보드가 실제 생성하는 최신 기본 설정 자체를 207턴에 사용한다.
...
- `npm run eval:questioning`: 통과, 49회기 207턴 0 failures, 0 review flags
...
- 자동평가의 질문 턴 비율은 31.9%다. 2국면 질문이 늘어난 만큼 실제 학생에게 부담스럽지 않은지 교사 블라인드 채점으로 확인해야 한다.
