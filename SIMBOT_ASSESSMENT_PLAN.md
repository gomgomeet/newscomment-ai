...
- 최대 5개. 미승인 초안은 자료 탐색모드에서 빈 필드도 보관할 수 있으나 자료형·크기·중복 ID는 항상 검사한다. 평가모드 저장에는 완성·승인된 계획이 필요하다.
...
- 기존 빈 계획과 미승인 계획은 읽어 교사가 복구할 수 있지만 평가모드의 준비·저장·진입을 차단하며 일반 대화로 대체하지 않는다. 탐색모드는 빈 계획과 초안을 저장·보존하되 학생 대화에는 실행하지 않는다.
