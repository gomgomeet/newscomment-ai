...
assert.equal(ui.byId('apply-assessment-draft').textContent, '입력란·질문계획에 적용');
...
const generatedPlan = readPlan(draftUi);
assert.equal(generatedPlan.approved, false, 'An AI-authored question plan always requires teacher approval');
assert.equal(generatedPlan.criteria.length, 1);
assert.equal(generatedPlan.criteria[0].criterion, draft.assessmentCriteria);
assert.equal(generatedPlan.criteria[0].mainQuestion, draft.startQuestion);
assert.equal(generatedPlan.criteria[0].sourceQuote, draft.assessmentEvidence);
assert.equal(generatedPlan.criteria[0].evidenceDescription, draft.evidenceDescription);
assert.equal(generatedPlan.criteria[0].requireSourceEvidence, true);
assert.match(generatedPlan.criteria[0].followUpQuestion, /\?$/);
...
blankCardUi.click('apply-assessment-draft');
assert.equal(readPlan(blankCardUi).criteria.length, 1, 'A blank manual card is replaced by the complete AI plan');
assert.equal(readPlan(blankCardUi).criteria[0].id, 'ai-criterion-1');
assert.ok(readPlan(blankCardUi).criteria[0].criterion);

const manualPlanUi = createUi({ ...fourSettings, assessmentPlanJson:JSON.stringify(planFixture) });
manualPlanUi.click('generate-assessment');
manualPlanUi.takeRequest('generateLiteMaterialAssessmentDraft').success({ ok:true, draft });
manualPlanUi.click('apply-assessment-draft');
assert.deepEqual(readPlan(manualPlanUi).criteria, planFixture.criteria, 'AI apply never overwrites a teacher-authored plan');
assert.equal(readPlan(manualPlanUi).approved, false, 'Changed assessment inputs revoke the old plan approval');
assert.match(manualPlanUi.byId('assessment-ai-status').textContent, /보존/);
...
assert.equal(explorationSave.args[1].startQuestion, draft.startQuestion, 'The hidden legacy draft survives changing to the shared exploration opening');
...
assert.equal(previewCopyUi.byId('preview-copy-fallback').tagName, 'details');
assert.equal(previewCopyUi.byId('preview-copy-fallback').classList.contains('hidden'), false, 'Manual copying is available without first detecting a clipboard failure');
assert.equal(previewCopyUi.byId('preview-copy-fallback').open, false, 'The direct-copy alternative starts collapsed');
...
assert.equal(previewCopyUi.byId('preview-copy-fallback').open, true, 'Copy failures expand the manual alternative');
assert.equal(previewCopyUi.byId('preview-url-fallback').value, readyData(settings).previewUrl);
assert.equal(previewCopyUi.byId('preview-url-fallback').getAttribute('readonly'), '');
...
assert.equal(previewCopyUi.byId('preview-copy-fallback').classList.contains('hidden'), false);
assert.equal(previewCopyUi.byId('preview-copy-fallback').open, false, 'Success collapses direct copying without removing it');
assert.equal(previewCopyUi.byId('preview-url-fallback').value, readyData(settings).previewUrl);
...
assert.equal(previewCopyUi.byId('preview-copy-fallback').open, false);
