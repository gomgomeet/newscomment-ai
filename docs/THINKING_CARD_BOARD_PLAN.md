...
| 단계 | 내용 | 검증 방법 |
| --- | --- | --- |
| 1 | 카드 8종 타입 + 로컬 카드 생성기 | 단위 실행으로 카드 JSON 확인 |
| 2 | 저장·조회 계층 | 로컬 Supabase 없이도 타입·쿼리 컴파일 확인 |
| 3 | API 라우트 | 빈 요청 400, 정상 요청 저장 |
| 4 | 확인 창 | Chrome 실기 — 메모 있을 때만 뜨는지, 없으면 한 번에 끝나는지 |
| 5 | Teacher AI 리서치 | 출처 없는 결과는 카드로 안 만드는지 |
| 6 | 학생 응답 검색 | 별도 작업 |
