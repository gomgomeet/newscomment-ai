...
const repairPattern = /(왜\s*자꾸|자꾸\s*물어|그만\s*물어|질문이\s*너무|부담스러|부담돼|재촉|싫다고|안\s*된다고)/;
const uncertainPattern = /(잘\s*모르|모르겠|몰라|글쎄|그냥\s*그런|생각\s*안|어려워)/;
const privacyOrCopyingPattern = /(전화번호|주소|비밀번호|주민번호|사진|이름은|답\s*다\s*써|그대로\s*써|대필|수행평가\s*답|숙제\s*해)/;
...
function recentStudentUncertainty(conversation: QuestioningConversationEntry[]) {
  return conversation
    ...
    .slice(-3)
    .filter((entry) => uncertainPattern.test(entry.content)).length;
}
...
export function decideQuestioningDialoguePolicy({
  ...
  if (closingPattern.test(compactTurn)) {
    return {
  ...
  if (uncertainPattern.test(studentTurn) || compactTurn.length <= 3) {
