...
| ~~6~~ | ~~학생 응답 검색~~ | 완료 — `findRelevantCards` + 실시간 리서치. 실키 대화로 확인 |
...
**1~6단계 모두 완료되어 `main`에 머지되었다**(PR #12, `aedd28a`).
요약은 `docs/THINKING_CARD_PR_SUMMARY.md` 참조.

계획 대비 달라진 것:

- **6단계를 이번 범위에 넣었다.** 원래 "카드가 쌓이는 것을 먼저 확인한 뒤"로 미뤘으나,
  실사용에서 "카드에 없던 질문" 수동 패널이 실효성이 없다는 지적이 나왔다. 카드를
  저장만 하고 챗봇이 읽지 않으면 아무 의미가 없어서, 답변 연결까지 한 번에 끝냈다.
- **실시간 리서치를 추가했다.** 원래는 "수업 중 검색은 하지 않는다"였지만, 자료 밖
  질문을 "없어요"로만 끝내는 것이 아이에게 답을 못 준다는 판단이 섰다. 대신 기준을
  카드보다 엄격하게 — A·B 출처 근거가 있을 때만, 없으면 기존의 정직한 답을 유지.
- **"카드에 없던 질문" 수동 답변 패널은 삭제했다.** 리서치가 자동으로 답하고 카드가
  챗봇에 실리는 지금 할 일이 없다. 백엔드(기록·교사 답변 카드 API)는 남아 있다.
- **004 마이그레이션 적용 완료**(표 5개 확인). 005(pgvector)는 아직 필요 없어 미적용.

### 위험 대비의 실제 결과

- ⑧이 느려지는 것 → 진행 표시로 대응, 실사용에서 문제 없었다
- 확인 창이 매번 뜨는 것 → 메모·리서치가 있을 때만 뜨도록 유지, 문제 없었다
- pgvector 미지원 → 004를 의존하지 않게 만든 것이 맞았다(적용 시 오류 없음)
- 003 적용 순서 → 합본 SQL(`supabase/manual-apply-003-004.sql`)로 해결
