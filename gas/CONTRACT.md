...
| 시트 | 열 |
| --- | --- |
| `CONFIG` | `key` `value` `description` — 키 11개: `APP_NAME` `GREETING_MESSAGE` `SUBJECT` `MAX_INPUT_LENGTH` `MIN_RELEVANCE_SCORE` `MAX_RETRIEVAL_RESULTS` `SHOW_EVIDENCE` `AI_ENABLED` `AI_MODEL` `AI_REASONING_EFFORT`(기본 low · none·low·medium·high·xhigh만, gpt-5.6-terra는 minimal을 HTTP 400으로 거부) `AI_MAX_OUTPUT_TOKENS` `AI_MAX_HISTORY_TURNS`(기본 4) `STUDENT_WEB_APP_URL` `LESSON_CODE` |
