...
- 작업 브랜치: `codex/vocabulary-context-chatbot`
...
평가 하네스는 가상 기사 15개를 사용한다. 개선에 사용하는 개발 세트 34회기 136턴과, 새 기사·학생 반응으로 구성한 홀드아웃 10회기 40턴을 분리해 실제 로컬 API에 재생한다. 개발 세트의 마지막 4회기 16턴은 잔반, 선택 배식, 공회전, 미세먼지, 흡음판, 데시벨, 다회용 질문을 다룬다.
...
- 44회기 결과는 시나리오 기반 합성 학습자 다중 턴 형성평가이며 실제 학습 효과 연구가 아니다.
- 로컬 경로는 범용 전자사전이 아니므로 교사가 검토한 어휘 표 밖의 임의 낱말은 제한적으로만 답한다.
- 홀드아웃도 한 차례 수정에 사용됐으므로 일반화 성능의 확정치로 해석할 수 없다.
- 자동평가 질문 턴 비율은 4.5%다. 교사 평가에서 질문 기회를 지나치게 줄였는지 확인해야 한다.
